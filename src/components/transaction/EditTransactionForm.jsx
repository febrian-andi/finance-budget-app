...
function EditTransactionForm({
  ...
  const { data: dataTransaction, loading: loadingDataTransaction } = useGetData(
    `${import.meta.env.VITE_API_URL}/expense-items/${idTransaction}`
  );
  ...
  useEffect(() => {
    if (dataTransaction) {
      setTitle(dataTransaction.title);
      setAmount(dataTransaction.amount);
      setWallet(dataTransaction.wallet);
      setCategory(dataTransaction.category);
      setFlowType(dataTransaction.flowType);
    }
  }, [dataTransaction]);
  ...
  if (loadingDataTransaction || loadingDataWallets || loadingDataCategories) {
    return (
