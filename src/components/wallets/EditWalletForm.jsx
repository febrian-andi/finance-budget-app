...
function EditWalletForm({ isOpenModal, toggleModal, refetchWallets, idWallet }) {
  if (!isOpenModal) return null;

  const [wallet, setWallet] = useState("");
  const [isLoading, setIsLoading] = useState(false);

  const { data, loading: loadingData } = useGetData(`${import.meta.env.VITE_API_URL}/wallets/${idWallet}`);
  ...
  const handleSubmit = (event) => {
    ...
    const requestData = { name: wallet };
    setIsLoading(true);

    putData(`${import.meta.env.VITE_API_URL}/wallets/${idWallet}`, requestData)
      .then(() => {
        refetchWallets();
        toggleModal();
      })
  ...
  return (
    <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md md:max-w-lg lg:max-w-xl mx-4">
        <h2 className="text-xl font-bold mb-4 border-b-2 border-gray-400 pb-2">Edit Wallet</h2>
